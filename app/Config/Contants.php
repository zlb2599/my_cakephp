<?php
$app_path = str_replace('\\', '/', dirname(dirname(__FILE__)));
define('MY_PRI', $app_path. "/Vendor/JDPay/rsa/360080003123050010.pfx");
define('MY_PUB', $app_path. "/Vendor/JDPay/rsa/npp_11_API2.pem");

class Contants {
	
	const httpurl="https://mapi.jdpay.com";//测试地址，外部商户请更换生产地址
	const signkey="360080003123050010abc";//签名key，测试环境测试的都是test，生产上一个会员对应一个key
	
	const  passWord="760184";//测试秘钥文件密码，随同pfx证书一起的密码
	const  pri=MY_PRI;//测试秘钥文件名（该文件包含公钥和私钥）外部商户请更换申请的pfx证书
	const  pub=MY_PUB;//代付证书文件名
	...
	static $IGNORE = array("sign_type", "sign_data", "encrypt_type", "encrypt_data", "salt");
	static $CODE = array(
					  "0000"=> "成功",
