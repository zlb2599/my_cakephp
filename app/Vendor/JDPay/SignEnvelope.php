...
class SignEnvelope{
	public static  function envelope($data){
		...
		//保存源数据文件
		$dataFile = dirname(__FILE__) . "/rsa/".$Millisecond."data.txt";
		//保存签名文件
		$signedFile = dirname(__FILE__) . "/rsa/".$Millisecond."signed.txt";
		//保存签名后base64文件
		$signedDataFile = dirname(__FILE__) . "/rsa/".$Millisecond."signedData.txt";
		//保存信封后文件
		$envelopeFile = dirname(__FILE__) . "/rsa/".$Millisecond."envelope.txt";
